...
from src.db.models import RawFile, Entry, DocumentLink, Job, OllamaServer, Worker
...
@app.get("/settings/get/{key}")
def get_setting_by_key(key: str, db: Session = Depends(get_db)):
    """Get a single setting by key."""
    ensure_settings_table(db)
    result = db.execute(text("SELECT value FROM settings WHERE key = :key"), {"key": key})
    row = result.fetchone()
    if not row:
        # Return default value if exists, otherwise 404
        if key in DEFAULT_SETTINGS:
            import json
            return {"key": key, "value": json.dumps(DEFAULT_SETTINGS[key])}
...
class KeyValueSettingUpdate(BaseModel):
    key: str
    value: str


@app.put("/settings/set")
def set_setting_by_key(update: KeyValueSettingUpdate, db: Session = Depends(get_db)):
...
# ============================================================================
# LLM Providers API - Registry of LLM providers (Ollama, OpenAI, Anthropic)
# ============================================================================
...
@app.post("/servers/test-connection")
...
async def list_servers(enabled_only: bool = False, db: Session = Depends(get_db)):
    """List all registered Ollama servers."""
...
@app.get("/servers/{server_id}/models")
...
@app.post("/servers/{server_id}/pull-model")
...
# ============================================================================
# Workers API - Registry of distributed workers
# ============================================================================
